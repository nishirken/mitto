...
describe('DialogRepository read markers', () => {
  ...
  beforeEach(async () => {
    ...
    hub = new DatabaseHub();
    repo = new DialogRepository(database, hub, new MediaRepository(database));
  ...
  test('advances the inbox marker and adopts the remaining unread count', async () => {
    ...
    expect(database.dialogs.get(peerId)).toMatchObject({ readInboxMaxId: 20, unreadCount: 1 });
  });

  test('advances the outbox marker without touching the unread count', async () => {
    await repo.applyReadOutbox(peerId, 12 as MessageId);

    expect(database.dialogs.get(peerId)).toMatchObject({ readOutboxMaxId: 12, unreadCount: 4 });
  });
  ...
  test('keeps the markers monotonic when updates arrive out of order', async () => {
    ...
    expect(database.dialogs.get(peerId)).toMatchObject({
      readInboxMaxId: 10,
      readOutboxMaxId: 6,
      unreadCount: 4,
    });
  ...
  test('ignores a read update for a dialog it has never seen', async () => {
    ...
    expect(database.dialogs.has('user:404')).toBe(false);
  });
});
